...
int ptrs_cmp(const void *a_ptr, const void *b_ptr){
    char *first_str = *(char**) a_ptr;
    char *second_str = *(char**) b_ptr;
    int first_iter = 0;
    int second_iter = 0;
...
int reverse_ptrs_cmp(const void *a_ptr, const void *b_ptr){
    const char * const first_str = *((char**)a_ptr);
    const char * const second_str = *((char**)b_ptr);
    
    int first_iter = 0;
    int second_iter = 0;
   
    while (first_str[first_iter + 1] != '\n'){
        first_iter++;
    }
    while (second_str[second_iter + 1] != '\n'){
        second_iter++;
    }
...
void calcHash(mVector *lines_ptrs_arr, char *book, long long start_iter, long long end_iter, bool reverse_flag){
    ...
    while (iter < end_iter && iter > 0)
    {
        ...
        while (iter < end_iter && iter > 0 && *(book + iter) != '\n')
        {
            ...
            {
                buff.hash[(hash_iter / 8)] |= (toupper(*(book + iter)) & 0x1F) << 3 >> (hash_iter % 8); //* Write 5 bit that placed in curent byte
                if (hash_iter % 8 > 3){
                    buff.hash[(hash_iter / 8) + 1] |= (toupper(*(book + iter)) & 0x1F) << 11 >> (hash_iter % 8); //* Write 5 bit that don't placed in curent byte
                }
